...
void Screen_getCalibData()
{
	...
	temperature_calibFloat_u16  =  getCalibData("calibEdit.n5.val");		// lay gia tri tu dia chi bien tren man hinh
...
// In cac du lieu tu man hinh ra Serial
	...
	Serial.println(temperature_calibFloat_u16 );		// in cac gia tri calib ra Serial
...
void Screen_displayCalibData()
{
	...
	myNex.writeNum("dl.n7.val", temperature_calibFloat_u16 );		// lay gia tri calib tren man hinh vao bien
	myNex.writeNum("dl.n8.val", humidity_calibFloat_u16     );		// lay gia tri calib tren man hinh vao bien
}


/**
 * @brief	ghi cac gia tri len man hinh, trinh bay man hinh
 *
 * @return  None
 */
void Screen_displayData()
{
	...
	{
		myNex.writeStr("dl.time.txt", String(realTime.now().toString("hh:mm DD-MMM-YY"))); 		// in thoi gian ra man hinh neu thoi gian hop le
	} else {
